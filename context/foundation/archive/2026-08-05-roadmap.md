...
# Reconciled 2026-08-05 against context/foundation/infrastructure.md and tech-stack.md, which were
# written after this roadmap was generated. Platform (Vercel Pro) and realtime transport (Ably) are
# decided; the session-state store remains open. Affected: F-01 unknowns, Open Roadmap Questions 1
# and 4. Slice sequencing is unchanged.
---
...
- **Blockers:** —
- **Unknowns:**
  - ~~Which durable store and which push transport carry a session?~~ **Partly resolved** by
    `context/foundation/infrastructure.md` and `tech-stack.md`, both written after this roadmap. The
    **push transport is decided: Ably**, a managed realtime provider, on Vercel Pro — chosen because
    Vercel's native WebSockets are public beta and cap connection duration below one 15-minute
    session, so an external provider is the only viable path. What remains open is the **durable
    store** for session state: Ably carries messages, not the authoritative scores, and in-memory
    state is not shared across serverless instances. Candidates recorded in `infrastructure.md` are a
    short-TTL store (Upstash Redis via the Vercel Marketplace) or Cloudflare Durable Objects, which
    would remove a vendor at the cost of a second platform. Owner: user. Block: yes — but narrowly,
    on the store only, not on the whole transport question.
  - Does the chosen combination hold at 150 concurrent devices within the 1-second budget, and at
    what running cost? — Owner: user (cost ceiling), measured by F-03. Block: no. Note Ably's free
    tier ceiling is 200 peak connections, leaving roughly 25% headroom at 150 participants.
...
1. **Where does session state live?** — *Narrowed on 2026-08-05, after `/10x-infra-research`.* As
   originally written this question read "where does session state live, and what pushes it to
   devices?", on the grounds that no stack decision was recorded. The second half is now answered:
   **Ably pushes state to devices**, hosted on Vercel Pro (see `infrastructure.md`, and
   `tech-stack.md` for the recorded contract). The remaining half is the durable store — Ably carries
   messages, not authoritative scores, and serverless instances share no memory, so a short-TTL store
   (Upstash Redis) or Cloudflare Durable Objects is still to be chosen. Owner: user. Block: F-01, and
   transitively S-02 through S-10 and F-02, F-03 — everything except S-01. Mirrors PRD Open
   Question 7.
...
3. **What is the acceptable running-cost ceiling now that cost neutrality is released?** — Released
   ...
   Question 3).
4. **How many weeks of work is full scope?** — Still unknown, so `timeline_budget.delivery_weeks` is
   unset. The platform and realtime layer are now decided, which removes part of the uncertainty; what
   remains hinges on the session-state store in Open Question 1. Owner: user, after Open Question 1
   resolves. Block: no (mirrors PRD Open Question 2).
